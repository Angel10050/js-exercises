...
var destinationNamesWithin500Kms = travelDestinations
  .filter(destino => destino.distanceKms < 500)
  .map(destino => destino.destinationName);

var destinationNameReachableByFerry = travelDestinations
  .filter(destino => destino.transportations.includes("ferry"))
  .map(destino => destino.destinationName);

var destinationNamesMoreThan300KmsAwayByTrain = travelDestinations
  .filter(destino => destino.transportations.includes("train"))
  .filter(destino => destino.distanceKms > 300)
  .map(destino => destino.destinationName);
